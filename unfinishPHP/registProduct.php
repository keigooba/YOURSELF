...
// 画面表示用データ取得
//================================
// GETデータを格納 elseの分岐処理を省略してるだけ urlを取得したいから p_idの情報を現在持っている
$p_id = (!empty($_GET['p_id'])) ? $_GET['p_id'] : '';
// DBから商品データを取得 セッションがもつp_idのデータを取得する（user_idをもつブラウザしか取得できない）
...
if(!empty($_POST)){
  ...
  // 画像をPOSTしてない（登録していない）が既にDBに登録されている場合、DBのパスを入れる（POSTには反映されないので） pic1にデータがない
  ...
  if(empty($dbFormData)){
    ...
    if($dbFormData['price'] != $price){ //前回まではキャストしていたが、ゆるい判定でもいい なぜ？
      //未入力チェック
      validRequired($price, 'price');
      //半角数字チェック
      validNumber($price, 'price');
          ...
          <form action="" method="post" class="form" enctype="multipart/form-data" style="width:100%;box-sizing:border-box;">
            ...
            <label class="<?php if(!empty($err_msg['category_id'])) echo 'err'; ?>">
              ...
              <select name="category_id" id="">
                ...
                <!-- selected 最初の選択肢を予め設定するもの -->
                <?php
                  foreach($dbCategoryData as $key => $val){
                    // 繰り返し処理を値に格納する $dbCategoryDataの変数を$valに格納
                ?>
